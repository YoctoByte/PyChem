from molecule.molecules import Molecule, Bond
...
def parse_from(molecule, smiles_string):
    ...
    if not isinstance(molecule, Molecule) and molecule is not None:
        raise ValueError(str(molecule) + ' not of type Molecule or None')
    ...
    for token in _tokenize(smiles_string):
        if token.lower() in ['b', 'c', 'n', 'o', 'p', 's', 'f', 'cl', 'br', 'i']:
            ...
            if not first_atom:
                first_atom = new_atom
            if active_atom:
                bonds.add(Bond(atoms={active_atom, new_atom}, type=bond_type))
            atoms.add(new_atom)
            active_atom = new_atom
            bond_type = '-'
        elif token[0] == '%':
            bonds.add(Bond(atoms={active_atom, token[1:]}, type=bond_type))
            bond_type = '-'
            ...
            bonds.add(Bond(atoms={active_atom, side_chain['first_atom']}, type=side_chain['first_bond_type']))
        elif token[0] == '[':
    ...
    _connect_bonds(atoms, bonds)
    _fill_hydrogen(atoms, bonds)

    if is_side_chain:
        side_chain = {'atoms': atoms, 'bonds': bonds, 'first atom': first_atom, 'first bond type': first_bond_type}
        return side_chain
    else:
...
def _parse_parenthesis(token):
    ...
    if index < len(token) and token[index] == '@':
        ...
        if index < len(token) and token[index] == '@':
            chirality += '@'
            index += 1
        if index+2 < len(token) and token[index:index+2] in ['TH', 'AL', 'SP', 'TB', 'OH']:
...
def _connect_bonds(atoms, bonds):
    labels = dict()
    new_bonds = set()
    bonds_to_remove = set()
